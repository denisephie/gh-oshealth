...
PUBLISH_LAG = timedelta(hours=2) # 2 hr buffer for file to exist (to clear misconception abt file uploaded/not and error)
...
def hour_range(start: datetime, end: datetime) -> Iterator[datetime]: #yield start to end utc hour (both inclusive)
    if start.tzinfo is None or end.tzinfo is None:
        raise ValueError("hour_range requires timezone-aware datetimes")
    cursor = start.astimezone(UTC).replace(minute=0, second=0, microsecond=0) #truncate bounds down to top of hr (00:45 treated as the hour of 00:00)
...
def is_expected_available(dt: datetime, *, now: datetime | None = None) -> bool: #shd this hour's file exist now, distinguishing unpublished and 404
