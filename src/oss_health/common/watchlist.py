...
from pydantic import (
    BaseModel,
    Field,
)  # basemodel to check types at construction, field to customize field behav
...
class Repo(BaseModel):  # pydantic model for tracked repo
    model_config = {
        "frozen": True
    }  # set immutable and hashable (repo obj lives in tuple/set as constants)
    ...
    )  # former name of repos -> if not given default to empty frozenset
...
WATCHLIST_NAMES: frozenset[str] = (
    frozenset(  # flatten watchlist into frozenset per name across all repos (check is this string in the watchlist w/o finding which repo it maps to)
...
def resolve(
    ...
    Repo | None
):  # look up owner/repo string from event (case insensitive) return matching repo or none
